...
public class FuncType extends BaseType {
    public DataType GetRetType() {return (DataType) GetChild(0);}
    public int GetNumParams() {return GetNumChildren()-1;}
    public VarAst GetParam(int i) {return (VarAst) GetChild(i+1);}
    public void AddParam(VarAst newpara) throws AssertionError {
        assert (GetNumChildren()>=1);
        AddChild(newpara);
    }
    
    FuncType(String nm) {super(nm);}
